...
function App() {
    ...
    const perPersonTip = numPeople > 0 ? twoDecimal(perPersonBill * (tipPercentage / 100)) : 0;
    const perPersonTotal = numPeople > 0 ? twoDecimal(perPersonBill + perPersonTip) : 0;
    ...
    // Reset bill and number of people
    const resetBill = () => {
        setBill("");
        setNumPeople("");
        setTipPercentage(0); // Reset the tip percentage
    };

    // Handle tip percentage button click
    const changeTipPercentage = (percentage) => {
        setTipPercentage(percentage); // Set the selected tip percentage
    };
                ...
                <div className="bg-White rounded-2xl md:p-6 p-8 flex md:flex-row flex-col justify-between gap-6">
                    <div className="md:w-[50%]">
                        ...
                        <div className="grid gap-3 md:grid-cols-3 grid-cols-2">
                            <button
                                onClick={() => changeTipPercentage(5)}  // Pass the percentage value
                                className={`rounded-lg px-4 py-2 mt-3 font-bold text-lg ${tipPercentage === 5 ? 'bg-Strongcyan text-White' : 'bg-Verydarkcyan text-White'}`}
                            >
                                5%
                            </button>
                            <button
                                onClick={() => changeTipPercentage(10)}
                                className={`rounded-lg px-4 py-2 mt-3 font-bold text-lg ${tipPercentage === 10 ? 'bg-Strongcyan text-White' : 'bg-Verydarkcyan text-White'}`}
                            >
                                10%
                            </button>
                            <button
                                onClick={() => changeTipPercentage(15)}
                                className={`rounded-lg px-4 py-2 mt-3 font-bold text-lg ${tipPercentage === 15 ? 'bg-Strongcyan text-White' : 'bg-Verydarkcyan text-White'}`}
                            >
                                15%
                            </button>
                            <button
                                onClick={() => changeTipPercentage(25)}
                                className={`rounded-lg px-4 py-2 mt-3 font-bold text-lg ${tipPercentage === 25 ? 'bg-Strongcyan text-White' : 'bg-Verydarkcyan text-White'}`}
                            >
                                25%
                            </button>
                            <button
                                onClick={() => changeTipPercentage(50)}
                                className={`rounded-lg px-4 py-2 mt-3 font-bold text-lg ${tipPercentage === 50 ? 'bg-Strongcyan text-White' : 'bg-Verydarkcyan text-White'}`}
                            >
                                50%
                            </button>
                            <input
                                ...
                                type="number"
                                placeholder="Custom"
                                onChange={(e) => changeTipPercentage(Number(e.target.value))} // Handle custom tip percentage
                            />
                        </div>
                        ...
                        <div>
                            <div className="flex justify-between items-center">
                                ...
                                {error && (
                                    <p className="text-red-500">Can't be zero</p>
                                )}
                            </div>
                            <div className="flex items-center justify-normal relative">
                                <img
                    ...
                    {/* Right Side (Total and Tip Calculation) */}
                    <div className="bg-Verydarkcyan p-6 rounded-xl flex flex-col gap-7">
                        ...
                        <button
                            className={` px-4 py-2 rounded-lg font-bold md:mt-16 mt-6 ${perPersonTip !== 0 && perPersonTotal !== 0 ? `bg-Strongcyan` : `bg-Darkgrayishcyan`}`}
                            onClick={resetBill}
                        >
                            RESET
                        </button>
